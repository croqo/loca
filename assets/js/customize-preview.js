/* global locaChipsBgColors, locaChipsPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( locaChipsBgColors ).forEach( function( context ) {
					locaChipsGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function locaChipsGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'locachips-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#locachips-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( locaChipsPreviewEls[ context ] ) {
				_.each( locaChipsPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		locaChipsGenerateColorA11yPreviewStyles( 'content' );
		locaChipsGenerateColorA11yPreviewStyles( 'header-footer' );
	} );

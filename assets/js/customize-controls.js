/* global locaChipsBgColors, locaChipsColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( locaChipsBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( locaChipsBgColors[ context ].color ) {
						backgroundColorValue = locaChipsBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( locaChipsBgColors[ context ].setting ).get();
					}
					locaChipsSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( locaChipsBgColors ).forEach( function( context ) {
			wp.customize( locaChipsBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					locaChipsSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		locaChipsSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				locaChipsSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function locaChipsSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = locaChipsColor( backgroundColor, accentHue );
	 ...
	 *
	 * @since Loca Chips 1.3
	 *
	 * @param {boolean} visible The visible value.
	 *
	 * @return {void}
	 */
	function locaChipsSetRetineLogoVisibility( visible ) {
